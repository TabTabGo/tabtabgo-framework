...
export declare type AuthenticateMode = 'phoneNumber';
export interface IAuthService {
    login: (username: string, password: string, redirectUrl?: string) => Promise<AuthenticateData>;
    loginByCode: (code: string, identification: any, mode: AuthenticateMode, onSuccess?: (result: any) => void) => Promise<AuthenticateData>;
