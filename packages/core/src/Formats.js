...
export const camel2title = (camelCase) => camelCase
    ? camelCase
        ...
        .replace(/_/g, () => ' ')
    : '';
export const numberFormat = (locale, decimalFixed) => (value) => {
    if (!locale)
        locale = AppSettings.defaultLocale || 'en-US';
    if (decimalFixed === undefined)
        decimalFixed = AppSettings.decimalFixed || 2;
...
export const amountFormat = (currency, locale, decimalFixed) => (amount) => {
    if (!currency)
...
export const genderFormat = (value, defaultValue) => {
    if (value === 'm' || value === 'male' || value === 1 || value === true)
        return 'Male';
    if (value === 'f' || value === 'female' || value === 0 || value === false)
        return 'Female';
    if (value === 'n')
        return 'Prefer not to say';
    return defaultValue || 'Not specified';
};
export const arrayFormat = (array, displayField, separator = ', ') => {
    if (array && Array.isArray(array)) {
        var displays = typeof displayField === 'function'
