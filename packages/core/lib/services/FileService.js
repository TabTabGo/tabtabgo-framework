...
var FileService = function FileService() {
  ...
  this.getImageUrl = function (url) {
    ...
    if (url && /^\/\d+$/.test(url)) {
      return "".concat(_Appsettings.AppSettings.baseApiUrl, "/files").concat(url, "/image/").concat(size);
    }
  ...
  this.getFileUrl = function (url) {
    ...
    if (url) {
      if (/^\/\d+$/.test(url)) return "".concat(_Appsettings.AppSettings.baseApiUrl, "/files").concat(url);else if (/^\/[a-zA-Z].*$/.test(url)) {
  ...
  this.uploadFile = /*#__PURE__*/function () {
    var _ref2 = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(blob, _ref, url) {
      var fileName, extension, fileInfo, data, prop;
      return regeneratorRuntime.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              fileName = _ref.fileName, extension = _ref.extension, fileInfo = _objectWithoutProperties(_ref, ["fileName", "extension"]);
              data = new FormData();
              ...
              if (!fileName) {
                fileName = "File_".concat((0, _moment["default"])().format('YYYYMMDDmmhhss'), ".").concat(extension);
              }

              data.append('formFile', blob, fileName);
              return _context.abrupt("return", _this.restApi.Post({
                url: url ? url : '/Create',
                isFormData: true,
                body: data,
