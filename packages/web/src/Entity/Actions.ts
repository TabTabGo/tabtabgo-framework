...
import { enqueueSnackbar } from '../components/Notifications/actions';
import { IEntityService } from 'ttg-react/core/Entity/Service';
import { Predicate } from 'ttg-react/core/types/Predicate';
import { ActionOptions } from 'ttg-react/core/types/ActionOptions';
import { OrderDirection } from 'ttg-react/core/types/enums/OrderDirection';
import { JsonPatch } from 'ttg-react/core/types/JsonPatch';
import { IActions } from 'ttg-react/core/types/Actions';

//import moment from "moment";
type MessageFunc = (result: any) => string;
/**
 * Module Action
 */
...
> implements IActions<T> {
  ...
  search = async (
    ...
    return this.currentService
      ...
      .catch((error) => {
        this.dispatch(this.changeFlag('loading', false));
        this.dispatch(this.changeFlag('searching', false));
  ...
  updateEntity = async (entity: T, options?: ActionOptions) => {
    ...
    try {
      ...
      var errorTitle = `Updating ${this.nameSingularText} ${
        this.getDisplayValue(entity) || ''
      } failed`;
  ...
  createEntity = async (entity: T, options?: ActionOptions) => {
    ...
    try {
      ...
      var errorTitle = `Adding ${this.nameSingularText} ${
        this.getDisplayValue(entity) || ''
      } failed`;
  ...
  deleteEntity = async (entity: T, options?: ActionOptions) => {
    try {
      ...
      var result = await this.currentService.deleteEntity(
        entity,
        options ? options.properties : null,
      );
      if (result) {
      ...
      let errorTitle = `Delete ${this.nameSingularText} ${
        this.getDisplayValue(entity) || ''
      } failed`;
  ...
  runAction = async (
    ...
    action: () => Promise<any>,
    onSuccessMessage?: string | boolean | MessageFunc,
    ...
    try {
      ...
      this.success(dispatch, actionType, onSuccessMessage, extraProps, result);
      return result;
    } catch (error) {
      this.error(dispatch, actionType, error, onFailedMessage, extraProps);
      if(extraProps?.throwError){
        throw error;
      }
    }
  };
  ...
  success = (
    dispatch: Dispatch,
    action: string | { [actionName: string]: any },
    message?: string | boolean | MessageFunc,
    extraProps = {},
    result?: any,
  ) => {
    dispatch(this.changeFlag(action, false, extraProps));
  ...
  destroy = () => {
    ...
    if (this.currentService) this.currentService.destroy();
    if (this.exportsService) this.exportsService.destroy();
  };
}
